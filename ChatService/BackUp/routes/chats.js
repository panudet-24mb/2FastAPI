...
chatsRouter.post('/', (req, res, next) => {
  console.log('test')
  chatsController
    .newChat(req.decoded.public_id, req.body)
    .then(({ success, message, chat }) => {
      console.log( success, message, chat )
...
chatsRouter.get('/my', (req, res, next) => {
  chatsController
    .getMyChats(req.decoded.public_id)
    .then(({ success, chats }) => {
      res.json({
        success,
        chats,
...
chatsRouter.get('/:id', (req, res, next) => {
  ...
  Promise.all([
    chatsController.getChat(req.decoded.public_id, req.params.id),
...
chatsRouter.post('/:id', (req, res, next) => {
  messagesController
    .sendMessage(req.decoded.public_id, req.params.id, req.body)
...
chatsRouter.get('/:id/join', (req, res, next) => {
  chatsController
    .joinChat(req.decoded.public_id, req.params.id)
